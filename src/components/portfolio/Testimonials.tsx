  ...
  return (
    <section className="relative py-28 px-6 lg:px-10">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex md:overflow-x-auto md:snap-x-strict no-scrollbar gap-6 flex-col md:flex-row pb-4">
          {TESTIMONIALS.map((t) => {
            const initials = t.name.split(" ").map((s) => s[0]).join("");
            return (
              <div
                key={t.name}
                className="snap-start shrink-0 md:w-[380px] bg-[#111] border border-[#1e1e1e] rounded-[12px] p-6 transition-all duration-300 hover:border-[rgba(170,255,69,0.5)] hover:-translate-y-1"
              >
